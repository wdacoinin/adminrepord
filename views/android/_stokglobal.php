...
$margin = [
    ...
    'value'=>function ($model, $key, $index, $widget) { //diffrent controller
        $hpp = $model->jml_now*$model->do_harga;
        $val = $model->jml_now*$model->harga_jual;
        $mrg = $val - $hpp;
        return $mrg;
    },
    'hAlign'=>'right', 
...
$columns = [
    ...
    [
        ...
        'value'=>function ($model, $key, $index, $widget) { //diffrent controller
            ...
            if($model->do_produk_origin > 0){
                return '<a class="btn btn-sm btn-primary" href="'. $ud . '">'. $model->produk . '-' . $model->do_produk_origin. '</a>';
            }else{
                return '<a class="btn btn-sm btn-primary" href="'. $ud . '">'. $model->produk . '-' . $model->do_produk. '</a>';
            }
        },
        'hAlign'=>'center', 
        'vAlign'=>'middle',
    ...
    [
        'label'=>'Value', 
        'value'=>function ($model, $key, $index, $widget) { //diffrent controller
            return $model->jml_now*$model->do_harga;
        },
        'hAlign'=>'right', 
        'vAlign'=>'middle',
        'width'=>'80px',
